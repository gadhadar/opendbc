...
Ecu = CarParams.Ecu
ESSENTIAL_ECUS = [Ecu.engine, Ecu.eps, Ecu.abs,
                  Ecu.fwdRadar, Ecu.fwdCamera, Ecu.vsa]
FUZZY_EXCLUDE_ECUS = [Ecu.fwdCamera, Ecu.fwdRadar, Ecu.eps, Ecu.debug]

FW_QUERY_CONFIGS: dict[str, FwQueryConfig] = get_interface_attr(
    'FW_QUERY_CONFIG', ignore_none=True)
VERSIONS = get_interface_attr('FW_VERSIONS', ignore_none=True)

MODEL_TO_BRAND = {c: b for b, e in VERSIONS.items() for c in e}
REQUESTS = [(brand, config, r)
            for brand, config in FW_QUERY_CONFIGS.items() for r in config.requests]
...
def match_fw_to_car_fuzzy(live_fw_versions: LiveFwVersions, match_brand: str = None, log: bool = True, exclude: str = None) -> set[str]:
    ...
    if match and len(matched_ecus) >= 2:
        if log:
            carlog.error(
                f"Fingerprinted {match} using fuzzy match. {len(matched_ecus)} matching ECUs")
        return {match}
    else:
        return set()
    ...
    for candidate, fws in candidates.items():
        ...
        for ecu, expected_versions in fws.items():
            expected_versions = expected_versions + \
                extra_fw_versions.get(candidate, {}).get(ecu, [])
            ecu_type = ecu[0]
            addr = ecu[1:]
    ...
    for exact_match, match_func in exact_matches:
        ...
        for brand in VERSIONS.keys():
            ...
            if not exact_match and not len(matches) and config.match_fw_to_car_fuzzy is not None:
                matches |= config.match_fw_to_car_fuzzy(
                    fw_versions_dict, vin, VERSIONS[brand])
...
def get_present_ecus(can_recv: CanRecvCallable, can_send: CanSendCallable, set_obd_multiplexing: ObdCallback, num_pandas: int = 1) -> set[EcuAddrBusType]:
    ...
    for obd_multiplexing in queries:
        ...
        for query in queries[obd_multiplexing]:
            ecu_responses.update(get_ecu_addrs(
                can_recv, can_send, set(query), responses, timeout=0.1))
...
def get_brand_ecu_matches(ecu_rx_addrs: set[EcuAddrBusType]) -> dict[str, set[AddrType]]:
    ...
    brand_matches: dict[str, set[AddrType]] = {
        brand: set() for brand, _, _ in REQUESTS}
...
def get_fw_versions(can_recv: CanRecvCallable, can_send: CanSendCallable, set_obd_multiplexing: ObdCallback, query_brand: str = None,
    ...
    car_fw = []
    requests = [(brand, config, r)
                for brand, config, r in REQUESTS if is_brand(brand, query_brand)]
                ...
                try:
                    ...
                    if query_addrs:
                        query = IsoTpParallelQuery(
                            can_send, can_recv, r.bus, query_addrs, r.request, r.response, r.rx_offset, debug=debug)
                        for (tx_addr, sub_addr), version in query.get_data(timeout).items():
                            f = CarParams.CarFw()

                            f.ecu = ecu_types.get(
                                (brand, tx_addr, sub_addr), Ecu.unknown)
                            f.fwVersion = version
                            f.address = tx_addr
                            f.responseAddress = uds.get_rx_addr_for_tx_addr(
                                tx_addr, r.rx_offset)
                            f.request = r.request
                            f.brand = brand
                            f.bus = r.bus
                            f.logging = r.logging or (
                                f.ecu, tx_addr, sub_addr) in config.extra_ecus
